...
This repository provides implementations of various PDEs including hyperbolic PDEs(Burger's Equation) and parabolic PDEs (Reaction-Diffusion/Heat Equations), and 2-D Navier Stokes PDEs that are all wrapped under a <a href=https://github.com/Farama-Foundation/Gymnasium>gym</a> designed for **boundary control problems**. The gym also includes the 1D1V Vlasov-Poisson system, which is instead a **distributed control problem**: the plasma is confined by an external electric field applied across a periodic domain rather than at a boundary (see the <a href=https://pdecontrolgym.readthedocs.io/en/latest/environments/vlasovpoisson-1d.html>environment documentation</a> for which parts of the phase-space state are reachable and which are provably not). These problems are particularly challenging because one has to handle nonlinearity across the entire domain with only a single value at each timestep requiring the solution of PDEs in the feedback loop. See the <a href=https://arxiv.org/abs/2405.11401>full paper</a> presented at L4DC 2024 for more details (Note the reward function implemented in the paper has been improved in the current version. See documentation reward for details). 
...
## Examples
Jupyter notebooks are made in the examples folder (Hyperbolic and Parabolic subfolders) that detail all the examples for both Hyperbolic and Parabolic PDEs. For the Vlasov-Poisson plasma environment, see `examples/vlasovPoisson/VlasovPoisson1DExample.ipynb`, which works through the instability analysis, both baselines, and a PPO training run. Additionally, full details on numerical implementation and solvers are available in the 
