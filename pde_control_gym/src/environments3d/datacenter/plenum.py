...
def build_plenum(layout, cells_per_tile, depth_m, beta, Q_sup_m3s):
    ...
    cfg = Config(dt=1.0, bcs=bcs, turb_model="chen", solve_energy=False,
                 outlet_mode="pressure", pressure_solver="amg")
...
def run_plenum(layout, cells_per_tile, depth_m, beta, Q_sup_m3s,
               omega=0.1, tol=1e-4, max_steps=2000, min_steps=20):
    ...
    result = PlenumResult(
        layout_name=layout.name, tile_ix=tile_ix, tile_iy=tile_iy,
        Q_m3s=Q_i, V_ms=V, p_kin=p_final, prd_m_pct=prd_m,
        prd_max=prd_max, prd_min=prd_min,
        prd_std=prd_std, steps=step, converged=converged,
        wall_time_s=wall_time_s)
    _save_csv(result, depth_m, beta, Q_sup_m3s)

    status = "CONVERGED" if converged else "NOT CONVERGED"
    print(f"plenum {layout.name} depth={depth_m*1000:.0f}mm beta={beta:.2f} "
...
def _save_csv(result, depth_m, beta, Q_sup_m3s):
    ...
    fname = (f"plenum_{result.layout_name}_d{round(depth_m*1000):d}"
    ...
    with open(path, "w", newline="") as f:
        ...
        for i in range(len(result.tile_ix)):
            w.writerow([result.tile_ix[i], result.tile_iy[i],
                        f"{result.Q_m3s[i]:.6f}", f"{result.V_ms[i]:.6f}",
                        f"{result.p_kin[i]:.6f}", f"{result.prd_m_pct[i]:.4f}"])
    return path
