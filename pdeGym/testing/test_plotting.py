...
class TestFigureObjClass(unittest.TestCase):
    ...
    def test_figure_defaults(self, mock_plt):
        ...
        assert mock_plt.figure().subfigures().__getitem__().subplots().plot.called == False
    ...
    def test_figure_no_error(self, mock_plt):
        ...
        figObj.makeFigure(self.x, self.t, self.u)
        assert mock_plt.figure.called
        mock_plt.figure().subplots().set_xlabel.assert_called_once_with(self.fig_settings["fig_xlabel"])
        mock_plt.figure().subplots().set_zlabel.assert_called_once_with(self.fig_settings["fig_zlabel"], rotation=90)
        mock_plt.figure().subplots().set_ylabel.assert_called_once_with(self.fig_settings["fig_ylabel"])
    ...
    def test_figure_with_error(self, mock_plt):
        ...
        figObj.makeFigure(self.x, self.t, self.u)
        assert mock_plt.figure.called
        assert mock_plt.figure().subfigures().__getitem__().subplots().plot_surface.called
